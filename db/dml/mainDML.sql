...
INSERT INTO ORDER_APPROVAL (APPROVAL_EMPLOYEE_ID, ORDER_REQUEST_ID, APPROVED_QUANTITY, APPROVAL_STATUS, APPROVAL_COMMENT)
VALUES
    -- 2번 데이터 (SYSTEM 무역센터점) 승인 내역
    ((SELECT EMPLOYEE_ID FROM EMPLOYEE WHERE LOGIN_ID = 'logi_system'),
     2, 12, 'APPROVED', '주말 행사 대비 전량 승인');

INSERT INTO ORDER_APPROVAL (APPROVAL_EMPLOYEE_ID, ORDER_REQUEST_ID, APPROVED_QUANTITY, APPROVAL_STATUS, APPROVAL_COMMENT)
VALUES
    -- 3번 데이터 (MINE 더현대서울점) 반려 내역
    ((SELECT EMPLOYEE_ID FROM EMPLOYEE WHERE LOGIN_ID = 'logi_mine'),
     3, NULL, 'REJECTED', '브랜드 물류센터 재고 부족으로 인한 반려');

INSERT INTO ORDER_APPROVAL (APPROVAL_EMPLOYEE_ID, ORDER_REQUEST_ID, APPROVED_QUANTITY, APPROVAL_STATUS, APPROVAL_COMMENT)
VALUES
    -- 4번 데이터 (THE CASHMERE 더현대서울점) 일부 승인 내역
    ((SELECT EMPLOYEE_ID FROM EMPLOYEE WHERE LOGIN_ID = 'logi_cashmere'),
     4, 5, 'APPROVED', '시즌 상품 재고 부족으로 6개 중 5개만 승인');
